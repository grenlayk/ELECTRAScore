...
class ELECTRAScorer:
    def __init__(self, device='cuda:0', max_length=1024, checkpoint="Aktsvigun/electra-large-cola"):
        # Set up model
        self.device = device
        self.max_length = max_length
        self.tokenizer = AutoTokenizer.from_pretrained(checkpoint)
        self.model = AutoModelForSequenceClassification.from_pretrained("Aktsvigun/electra-large-cola")
        self.model.eval()
    ...
    def score(
        ...
        tokenized_data = Dataset.from_dict({"text": texts}).map(tokenize_fn, remove_columns=["text"], batched=True)
        ...
        probas = torch.empty(len(texts), dtype=torch.float32, device=self.device)
        sent_probas = torch.empty(len(sentences), dtype=torch.float32, device=self.device)

        start = 0
        end = batch_size
        with torch.no_grad():
            for i, batch in enumerate(tqdm(dataloader)):
                batch_pred = self.model(**{k: v.to(self.device) for k, v in batch.items()})
